...
def EBM(X, y, dataset_name, model_name='EBM'):
    if task == "classification":
         ebm = ExplainableBoostingClassifier(interactions=20, max_bins=512, outer_bags=16, inner_bags=0)
    else:
        ebm = ExplainableBoostingRegressor(interactions=20, max_bins=512, outer_bags=16, inner_bags=0)
    ebm.fit(X, y)
...
def Gaminet(X, y, dataset_name, model_name='Gaminet'):
    ...
    if task == "classification":  # 190 it/ s Maineffect, 65 it/s Interaction
        model_to_run = GAMINet(meta_info=meta_info, verbose=True)
