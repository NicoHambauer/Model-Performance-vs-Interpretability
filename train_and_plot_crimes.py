...
def EBM(X, y, dataset_name, model_name='EBM'):
    if task == "classification":
         ebm = ExplainableBoostingClassifier(interactions=20, max_bins=512, outer_bags=16, inner_bags=4)
    else:
        ebm = ExplainableBoostingRegressor(interactions=20, max_bins=512, outer_bags=16, inner_bags=4)
    ebm.fit(X, y)
...
def PYGAM(X, y, dataset_name, model_name='PYGAM'):
    ...
    tms = terms.TermList(
        *[f(i) if X.columns[i] in dataset.categorical_cols else s(i, n_splines=5,
                                                             lam=0.9) for i in
                  range(X.shape[1])])
...
def LR(X, y, dataset_name, model_name='LR'):
    if task == 'regression':
        m = ElasticNet(alpha=0.01, l1_ratio=0.7)
    else:
