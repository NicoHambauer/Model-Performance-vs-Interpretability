...
class MyRSplineBase(MyExtractLogOddsMixin, OnehotEncodingFitMixin):
    def __init__(
        ...
        **kwargs,
        ...
        assert not (
            model_to_use == "gam" and discrete is True
        ), "Not supported for discrete GAM"
    ...
    def fit(self, X, y, **kwargs):
        ...
        formula_terms = []
        for feat_name, clean_feat_name in zip(
            self.feature_names, self.clean_feature_names
        ):
            ...
            """ term_str = (
                ...
                % (
                    clean_feat_name,
                    self.spline_type,
                    min(self.maxk, int(num_unique_x * 2 / 3)),
                )
            ) """

            if num_unique_x == 2 or num_unique_x == 3:
                term_str = "%s" % clean_feat_name
            else:
                term_str = "s(%s, bs='%s', k=%d, m=%d)" % (
                    clean_feat_name,
                    self.spline_type,
                    min(self.maxk, int(num_unique_x)),
                    self.m,
                )
        ...
        self.R_model = ro.r[self.model_to_use](
            formula,
            family=self.family,
            gamma=self.gamma,
    ...
    def get_GAM_plot_dataframe(self, x_values_lookup=None):
        ...
        assert (
            self.GAM_plot_dataframe is not None
        ), "Call create_df_from_R_model() first!"
    ...
    def create_df_from_R_model(self, X):
        ...
        self.GAM_plot_dataframe = self.revert_dataframe(
            super().get_GAM_plot_dataframe(x_values_lookup)
        )
