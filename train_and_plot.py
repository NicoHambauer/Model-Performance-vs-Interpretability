...
def EBM(X, y, dataset_name, model_name='EBM'):
    if task == "classification":
         ebm = ExplainableBoostingClassifier(interactions=2, max_bins=512, outer_bags=16, inner_bags=4)
    else:
        ebm = ExplainableBoostingRegressor(interactions=2, max_bins=512, outer_bags=16, inner_bags=4)
        # epsilon, delta
    ebm.fit(X, y)
...
def PYGAM(X, y, dataset_name, model_name='PYGAM'):
    ...
    tms = terms.TermList(
        *[f(i) if X.columns[i] in dataset.categorical_cols else s(i, n_splines=20,
                                                             lam=0.6) for i in
                  range(X.shape[1])])
...
def Gaminet(X, y, dataset_name, model_name='Gaminet'):
    ...
    if task == "classification":  # 190 it/ s Maineffect, 65 it/s Interaction
        model_to_run = GAMINet(meta_info=meta_info, interact_num=20,
                               ...
                               heredity=False, loss_threshold=0.01, reg_clarity=0.1,  # clarity 1.0
    ...
    feature_importance_visualize(feat_for_vis, save_png=True, folder='.', name='gaminet_feat_imp', n_features=10)
...
def LR(X, y, dataset_name, model_name='LR'):
    if task == 'regression':
        m = ElasticNet(alpha=1, l1_ratio=0)
    else:
        m = LogisticRegression(C=0.1, penalty='l2',
                                              class_weight=None,
                                              solver='lbfgs', l1_ratio= None, max_iter=100, n_jobs=-1)
    m.fit(X, y)
    plot_data = PlotData()
