...
def make_one_hot_multi_plot(
    plot_data: PlotData, model_name, dataset_name, distibution_plot=True
):
...
def EBM(X, y, dataset_name, model_name="EBM"):
    ...
    for i, _ in enumerate(ebm_global.data()["names"]):  # shape_name besserer Name?
        ...
        if shape_data["type"] == "interaction":
            ...
            if (
                len(shape_data["left_names"]) == 2
                and len(shape_data["right_names"]) > 2
            ):
                # plot_pairwise_heatmap(shape_data, x_name, y_name, model_name, dataset_name)
                make_plot_interaction_onehot_line_ebm(
                    shape_data, x_name, y_name, model_name, dataset_name
                )
            elif (
                len(shape_data["left_names"]) > 2
                and len(shape_data["right_names"]) == 2
                ...
                raise ValueError(
                    "Left Names contains continue values. We thought that should not happen. Fix it."
                )
            elif (
                len(shape_data["left_names"]) == 2
                and len(shape_data["right_names"]) == 2
                ...
                make_plot_interaction(
                    shape_data["left_names"],
                    shape_data["right_names"],
    ...
    feature_importance_visualize(
        ...
        save_png=True,
        folder=".",
        name=f"ebm_feat_imp_{dataset_name}",
        n_features=10,
    )

    # ebm_local_explainer = EBMLocalExplainer(ebm, X[:50], y[:50])
    # ebm_local_explainer.add_number_word_filter(150, "hr", exact_match=True)03,
...
def Gaminet(X, y, dataset_name, model_name="Gaminet"):
    ...
    for effect in data_dict.keys():
        if data_dict[effect]["type"] == "pairwise":
            x_name, y_name = effect.split(" x ")

            if (
                len(data_dict[effect]["input1"]) == 2
                and len(data_dict[effect]["input2"]) > 2
            ):  # if there is continous x categirical
                make_plot_interaction_onehot_line_gaminet(
                    data_dict[effect]["input1"],
                    data_dict[effect]["input2"],
        ...
        # Classical one hot plot
            ...
            data_dict[effect]["type"] == "categorical"
            and X_original[effect.split("_")[0]].value_counts().size == 2
        ):
            make_one_hot_plot(
                data_dict[effect]["outputs"][0],
                data_dict[effect]["outputs"][-1],
                Xnames2Featurenames[effect],
                model_name,
                dataset_name,
            )
    ...
    for i, effect in enumerate(data_dict.keys()):
        if "vs." in effect:
            ...
            feat_for_vis[Xnames2Featurenames[effect]] = {
                "importance": data_dict[effect]["importance"]
            }
    ...
    feature_importance_visualize(
        ...
        save_png=True,
        folder=".",
        name=f"gaminet_feat_imp_{dataset_name}",
        n_features=10,
    )
...
def MGCV(X, y, dataset_name, model_name="MGCV"):
    ...
    shape_data = []
    for feat_idx, feat_name in enumerate(X.columns.values.tolist()):
        shape_data.append(
            {
                "name": feat_name,
                "x": torch.tensor(df["x"].tolist()[1:][feat_idx]),
                "y": torch.tensor(df["y"].tolist()[1:][feat_idx]),
                "importance": torch.tensor(df["importance"].tolist()[1:][feat_idx]),
            }
        )
